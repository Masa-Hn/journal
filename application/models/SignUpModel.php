...
class SignUpModel extends CI_Model {
	...
	//ANY LEADER
	public function newTeamsAnyLeader($ambassador_gender)
	{
		$sql = "SELECT leader_request.Rid,leader_request.members_num, leader_request.date, leader_request.is_done, leader_request.leader_id, leader_request.gender, leader_info.leader_gender FROM leader_request INNER JOIN leader_info ON leader_request.leader_id = leader_info.id WHERE leader_request.is_done = 0 AND leader_request.current_team_count = 0 AND leader_request.gender = '".$ambassador_gender."' AND (leader_info.leader_gender = 'female' OR leader_info.leader_gender = 'male') ORDER BY leader_request.date ASC LIMIT 1";
		$query = $this->db->query($sql);
	    return $query->row();

	}//getNewTeams

	public function anyLeader($ambassador_gender,$logical_operator)
	{
		$sql = "SELECT leader_request.Rid,leader_request.members_num, leader_request.current_team_count, leader_request.is_done, leader_request.leader_id, leader_request.gender, leader_info.leader_gender FROM leader_request INNER JOIN leader_info ON leader_request.leader_id = leader_info.id WHERE leader_request.is_done = 0 AND leader_request.current_team_count ".$logical_operator." 12 AND leader_request.gender = '".$ambassador_gender."' AND (leader_info.leader_gender = 'female' OR leader_info.leader_gender = 'male') ORDER BY leader_request.date ASC LIMIT 1";
		$query = $this->db->query($sql);
	    return $query->row();
	...
	public function getNewTeams($Leader_gender,$ambassador_gender)
	{
		$sql = "SELECT leader_request.Rid,leader_request.members_num, leader_request.date, leader_request.is_done, leader_request.leader_id, leader_request.gender, leader_info.leader_gender FROM leader_request INNER JOIN leader_info ON leader_request.leader_id = leader_info.id WHERE leader_request.is_done = 0 AND leader_request.current_team_count = 0 AND leader_request.gender = '".$ambassador_gender."' AND leader_info.leader_gender = '" .$Leader_gender."' ORDER BY leader_request.date ASC LIMIT 1";
		$query = $this->db->query($sql);
	    return $query->row();

	}//getNewTeams

	public function getTeams($Leader_gender,$ambassador_gender,$logical_operator)
	{
		$sql = "SELECT leader_request.Rid,leader_request.members_num, leader_request.current_team_count, leader_request.is_done, leader_request.leader_id, leader_request.gender, leader_info.leader_gender FROM leader_request INNER JOIN leader_info ON leader_request.leader_id = leader_info.id WHERE leader_request.is_done = 0 AND leader_request.current_team_count ".$logical_operator." 12 AND leader_request.gender = '".$ambassador_gender."' AND leader_info.leader_gender = '".$Leader_gender. "' ORDER BY leader_request.date ASC LIMIT 1";
		$query = $this->db->query($sql);
	    return $query->row();
