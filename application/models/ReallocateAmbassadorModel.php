...
class ReallocateAmbassadorModel extends CI_Model {
	...
	public function selectHighPriority($leader_gender,$ambassador_gender){
		$sql = "SELECT leader_request.Rid,leader_request.members_num,leader_request.is_done, leader_request.leader_id, leader_request.gender, leader_info.leader_gender FROM leader_request INNER JOIN leader_info ON leader_request.leader_id = leader_info.id INNER JOIN high_priority_requests ON leader_request.Rid = high_priority_requests.request_id WHERE ".$leader_gender." AND ".$ambassador_gender." AND leader_request.is_done = 0 AND leader_info.leader_rank NOT like '%22%' ORDER BY high_priority_requests.id ASC LIMIT 1";
	...
	public function checkAmbassadorCode($code){
